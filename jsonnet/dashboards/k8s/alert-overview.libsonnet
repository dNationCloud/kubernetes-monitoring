  ...
  grafanaDashboards+:: {
    'alert-cluster-overview':
      ...
      local alertsInfoTable =
        ...
        .addTargets([
          prometheus.target('ALERTS{cluster=~"$cluster", alertname!="Watchdog", alertstate=~"firing|pending", severity=~"$severity", alertgroup=~"$alertgroup"} * 2', format='table', instant=true),
          prometheus.target('ALERTS{cluster=~"$cluster", alertname!="Watchdog", alertstate=~"firing|pending", severity=~"$severity", alertgroup=~"$alertgroup"} * 3', format='table', instant=true),
        ])
        .addTransformations([
          {
            id: 'organize',
