  ...
  grafanaDashboards+:: {
    'alert-host-overview':
      ...
      local alertsInfoTable =
        ...
        .addTargets([
          prometheus.target('ALERTS{job=~"$job", alertname!="Watchdog", alertstate=~"firing|pending", severity=~"$severity", alertgroup=~"$alertgroup"} * 2', format='table', instant=true),
          prometheus.target('ALERTS{job=~"$job", alertname!="Watchdog", alertstate=~"firing|pending", severity=~"$severity", alertgroup=~"$alertgroup"} * 3', format='table', instant=true),
        ])
        .addTransformations([
          {
            id: 'organize',
