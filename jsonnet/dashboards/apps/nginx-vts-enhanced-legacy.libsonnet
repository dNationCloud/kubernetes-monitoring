  ...
  grafanaDashboards+:: {
    'nginx-vts-enhanced-legacy':
      ...
      local serverConnections =
        ...
        + timeSeriesPanel.queryOptions.withTargets([
          promTarget('sum(nginx_server_connections{cluster="$cluster", job=~"$job", namespace=~"$namespace", pod=~"$pod", status=~"active|writing|reading|waiting"}) by (status)', '{{status}}'),
        ]);
      ...
      local serverCache =
        ...
        + timeSeriesPanel.standardOptions.withMin(0)
        + timeSeriesPanel.queryOptions.withTargets([
          promTarget('sum(irate(nginx_server_cache{cluster="$cluster", job=~"$job", namespace=~"$namespace", pod=~"$pod", host=~"^$host$"}[5m])) by (status)', '{{status}}'),
        ]);

      local serverRequests =
        timeSeriesBase('Server Requests')
        + timeSeriesPanel.queryOptions.withTargets([
          promTarget('sum(irate(nginx_server_requests{cluster="$cluster", job=~"$job", namespace=~"$namespace", pod=~"$pod", host=~"^$host$", code!="total"}[5m])) by (code)', '{{code}}'),
        ]);
      ...
      local serverBytes =
        ...
        + timeSeriesPanel.standardOptions.withMin(0)
        + timeSeriesPanel.queryOptions.withTargets([
          promTarget('sum(irate(nginx_server_bytes{cluster="$cluster", job=~"$job", namespace=~"$namespace", pod=~"$pod", host=~"^$host$"}[5m])) by (direction)', '{{direction}}'),
        ]);
      ...
      local upstreamRequests =
        ...
        + timeSeriesPanel.queryOptions.withTargets([
          promTarget('sum(irate(nginx_upstream_requests{cluster="$cluster", job=~"$job", namespace=~"$namespace", pod=~"$pod", upstream=~"^$upstream$", code!="total"}[5m])) by (code)', '{{code}}'),
        ]);
      ...
      local upstreamBytes =
        ...
        + timeSeriesPanel.standardOptions.withMin(0)
        + timeSeriesPanel.queryOptions.withTargets([
          promTarget('sum(irate(nginx_upstream_bytes{cluster="$cluster", job=~"$job", namespace=~"$namespace", pod=~"$pod", upstream=~"^$upstream$"}[5m])) by (direction)', '{{direction}}'),
        ]);

      local upstreamBackendResponse =
        timeSeriesBase('Upstream Backend Response')
        + timeSeriesPanel.queryOptions.withTargets([
          promTarget('sum(nginx_upstream_responseMsec{cluster="$cluster", job=~"$job", namespace=~"$namespace", pod=~"$pod", upstream=~"^$upstream$"}) by (backend)', '{{backend}}'),
        ]);
      ...
      local templates =
        ...
        + [
          ...
          $.grafanaTemplates.baseTemplate('host', 'Host', 'label_values(nginx_server_bytes{cluster="$cluster", job=~"$job", namespace=~"$namespace", pod=~"$pod"}, host)'),
          $.grafanaTemplates.baseTemplate('upstream', 'Upstream', 'label_values(nginx_upstream_bytes{cluster="$cluster", job=~"$job", namespace=~"$namespace", pod=~"$pod"}, upstream)'),
        ]
      ...
      dashboard.new('Nginx VTS Enhanced Legacy')
      + dashboard.withUid($._config.grafanaDashboards.ids.nginxVtsEnhancedLegacy)
