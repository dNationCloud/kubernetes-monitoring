    ...
    'nginx-vts-legacy':
      ...
      local serverConnections =
        ...
        + timeSeriesPanel.queryOptions.withTargets([
          prometheus.withExpr('sum(nginx_server_connections{cluster="$cluster", job=~"$job", namespace=~"$namespace", pod=~"$pod", status=~"active|writing|reading|waiting"}) by (status)') + prometheus.withLegendFormat('{{status}}'),
        ]);

      local serverRequests =
        timeSeriesBase('Server Requests')
        + timeSeriesPanel.queryOptions.withTargets([
          prometheus.withExpr('sum(irate(nginx_server_requests{cluster="$cluster", job=~"$job", namespace=~"$namespace", pod=~"$pod", host=~"^$host$", code!="total"}[5m])) by (code)') + prometheus.withLegendFormat('{{code}}'),
        ]);
      ...
      local serverBytes =
        ...
        + timeSeriesPanel.queryOptions.withTargets([
          prometheus.withExpr('sum(irate(nginx_server_bytes{cluster="$cluster", job=~"$job", namespace=~"$namespace", pod=~"$pod", host=~"^$host$"}[5m])) by (direction)') + prometheus.withLegendFormat('{{direction}}'),
        ]);
      ...
      dashboard.new('Nginx VTS Legacy')
      + dashboard.withUid($._config.grafanaDashboards.ids.nginxVtsLegacy)
