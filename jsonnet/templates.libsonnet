    ...
    k8s: {
      ...
      totalDisk: {
        panel: {
          ...
          graphMode: 'none',
          unit: 'bytes',
          expr: 'sum(node_filesystem_size_bytes{cluster=~"$cluster", %(job)s, device!="rootfs"})' % { job: 'job=~"$job"' },
          thresholds: { color: $.defaultConfig.grafanaDashboards.color.white, value: null },
          gridPos: {
            x: 15,
            y: 20,
